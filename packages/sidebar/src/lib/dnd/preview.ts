...
export function calculateInsertPosition<T>(options: {
...
export function computePreviewInsert<T>(options: {
	livePreview: boolean;
	draggedItem: PreviewDraggedItem<T> | null;
	...
	if (!draggedItem || !dropTargetId || !dropPosition) return null;
...
export function isPreviewItem<T>(options: {
	livePreview: boolean;
	draggedItem: PreviewDraggedItem<T> | null;
...
export function getItemsWithPreview<T>(options: {
	items: T[];
	parentId: string | null;
	draggedItem: PreviewDraggedItem<T> | null;
	previewInsert: PreviewInsertPosition | null;
